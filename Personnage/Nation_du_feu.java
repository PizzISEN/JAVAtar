...
public class Nation_du_feu extends Humain {
    ...
    public int rencontre(Humain h){
        ...
        if(h.getEquipe() != this.getEquipe()) {
            ...
            // Si le lancé de dé personnel est supérieur au lancé adverse ou que l'ennemi fait partie de l'équipe de l'air en cas d'égalité
            if(dePerso > deAdverse || (dePerso == deAdverse && h.getEquipe() == "A")) {
                // L'adversaire meurt
                h.mort();
                // Sa case devient un obstacle
                return 1;
            // Si le lancé de dé personnel est inférieur au lancé adverse ou que l'ennemi fait partie de l'équipe de l'eau en cas d'égalité
            } else if(dePerso < deAdverse || (dePerso == deAdverse && h.getEquipe() == "E")) {
                // L'humain meurt
                this.mort();
                // Sa case devient un obstacle
                return 0;
            // Si l'adversaire fait partie de l'équipe de la terre et qu'il y a égalité
            } else {
