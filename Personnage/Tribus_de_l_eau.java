...
public class Tribus_de_l_eau extends Humain{
    ...
    public int rencontre(Humain h){
        ...
        if(h.getEquipe() != this.getEquipe()) {
            ...
            // Si le lancé de dé personnel est supérieur au lancé adverse ou que l'ennemi fait partie de l'équipe du feu en cas d'égalité
            if(dePerso > deAdverse || (dePerso == deAdverse && h.getEquipe() == "F")) {
                // L'adversaire meurt
                h.mort();
                // Sa case devient un obstacle
                return 1;
            // Si le lancé de dé personnel est inférieur au lancé adverse ou que l'ennemi fait partie de l'équipe de la terre en cas d'égalité
            } else if(dePerso < deAdverse || (dePerso == deAdverse && h.getEquipe() == "T")) {
                // L'humain meurt
                this.mort();
                // Sa case devient un obstacle
                return 0;
            // Si l'adversaire fait partie de l'équipe de l'air et qu'il y a égalité
            } else {
